...
import com.cdqckj.gmis.file.dto.chunk.FileChunkCheckDTO;
import com.cdqckj.gmis.file.dto.chunk.FileChunksMergeDTO;
import com.cdqckj.gmis.file.dto.chunk.FileUploadDTO;
import com.cdqckj.gmis.file.entity.SystemFile;
import com.cdqckj.gmis.file.manager.WebUploader;
...
public class FileChunkController {
    ...
    public R<Boolean> saveMd5Check(@RequestParam(name = "md5") String md5,
        ...
        SystemFile file = fileChunkStrategy.md5Check(md5, folderId, accountId);
    ...
    public R<String> uploadFile(FileUploadDTO info, @RequestParam(value = "file", required = false) MultipartFile file) throws Exception {
        ...
        if (file == null || file.isEmpty()) {
            log.error("请求参数不完整");
            return R.fail("请求参数不完整","Incomplete request parameters");
        }
        ...
        */
        if (info.getChunks() == null || info.getChunks() <= 0) {
            SystemFile upload = fileStrategy.upload(file, null);
            ...
            if (target == null) {
                return R.fail(wu.getErrorMsg(),wu.getErrorMsgEn());
            }
            //保存上传文件
    ...
    @ResponseBody
    @SysLog("上传大文件")
    public R<SystemFile> saveChunksMerge(@RequestBody FileChunksMergeDTO info) {
        log.info("info={}", info);
