...
import com.cdqckj.gmis.file.dao.FileMapper;
...
import com.cdqckj.gmis.file.domain.FileStatisticsDO;
import com.cdqckj.gmis.file.dto.FileOverviewDTO;
import com.cdqckj.gmis.file.dto.FileStatisticsAllDTO;
...
@Service
@DS("#thread.tenant")
public class FileServiceImpl extends SuperServiceImpl<FileMapper, SystemFile> implements FileService {
    ...
    @Override
    public SystemFile upload(MultipartFile simpleFile, Long folderId) {
        ...
        SystemFile file = fileStrategy.upload(simpleFile,null);
    ...
    public FileAttrDO getFileAttrDo(Long folderId) {
        ...
        SystemFile folder = this.getById(folderId);
        ...
        BizAssert.isTrue(grade <= 10, BASE_VALID_PARAM.build("文件夹层级不能超过10层","Folder level cannot exceed 10"));
    ...
    public FolderDTO saveFolder(FolderSaveDTO folderSaveDto) {
        SystemFile folder = BeanPlusUtil.toBean(folderSaveDto, SystemFile.class);
        if (folderSaveDto.getFolderId() == null || folderSaveDto.getFolderId() <= 0) {
            ...
            SystemFile parent = super.getById(folderSaveDto.getFolderId());
            BizAssert.notNull(parent, BASE_VALID_PARAM.build("父文件夹不能为空","Parent folder cannot be empty"));
            BizAssert.isFalse(parent.getIsDelete(), BASE_VALID_PARAM.build("父文件夹已经被删除","The parent folder has been deleted"));
            BizAssert.equals(DataType.DIR.name(), parent.getDataType().name(), BASE_VALID_PARAM.build("父文件夹不存在","Parent folder does not exist"));
            BizAssert.isTrue(parent.getGrade() < 10, BASE_VALID_PARAM.build("文件夹层级不能超过10层","Folder level cannot exceed 10"));
    ...
    public boolean removeFile(Long[] ids, Long userId) {
        LbuWrapper<SystemFile> lambdaUpdate =
                Wraps.<SystemFile>lbU()
                        .in(SystemFile::getId, ids)
                        .eq(SystemFile::getCreateUser, userId);
        SystemFile file = SystemFile.builder().isDelete(Boolean.TRUE).build();
    ...
    public Boolean removeList(Long userId, List<Long> ids) {
        ...
        List<SystemFile> list = super.list(Wrappers.<SystemFile>lambdaQuery().in(SystemFile::getId, ids));
        if (list.isEmpty()) {
            return true;
        }
    ...
    public void download(HttpServletRequest request, HttpServletResponse response, Long[] ids, Long userId) throws Exception {
        ...
        List<SystemFile> list = (List<SystemFile>) super.listByIds(Arrays.asList(ids));
