package com.cdqckj.gmis.general.controller;
...
import com.cdqckj.gmis.authority.enumeration.common.LogType;
...
import com.cdqckj.gmis.msgs.enumeration.MsgsBizType;
import com.cdqckj.gmis.msgs.enumeration.MsgsCenterType;
import com.cdqckj.gmis.security.annotation.PreAuth;
import com.cdqckj.gmis.sms.enumeration.ProviderType;
import com.cdqckj.gmis.sms.enumeration.SendStatus;
...
public class OauthGeneralController {
    ...
    @Value("${gmis.database.isNotWrite:false}")
    private Boolean isNotWrite;

    @ApiOperation(value = "获取当前系统指定枚举", notes = "获取当前系统指定枚举")
    @GetMapping("/enums")
    public R<Map<String, Map<String, String>>> enums(@RequestParam(value = "codes") String codes) {
...
//        }
    ...
    @ApiOperation(value = "获取当前系统指定枚举--新的方式", notes = "获取当前系统指定枚举--新的方式")
    @PostMapping("/enumList")
    public R<Map<String, Map<String, String>>> enumList(@RequestBody List<String> codes) {
        if (codes == null || codes.size() == 0) {
            return R.success(ENUM_MAP);
        }

        Map<String, Map<String, String>> map = new HashMap<>(codes.size());
