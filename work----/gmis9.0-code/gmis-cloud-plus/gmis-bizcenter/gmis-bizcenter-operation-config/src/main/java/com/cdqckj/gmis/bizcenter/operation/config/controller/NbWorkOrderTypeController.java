...
@Validated
@RestController
@RequestMapping("/operparam/nbworkordertype")
@Api(value = "nbworkordertype", tags = "")
//@PreAuth(replace = "street:")
public class NbWorkOrderTypeController {
    @Autowired
    public CommonConfigurationApi commonConfigurationApi;
    @ApiOperation(value = "新增报装工单类型配置")
    @PostMapping("/dictionaryItem/add")
    public R<DictionaryItem> saveNbWorkOrderType(@RequestBody DictionaryItemSaveDTO dictionaryItemSaveDTO){
        return commonConfigurationApi.save(dictionaryItemSaveDTO);
    }

    @ApiOperation(value = "更新报装工单类型配置")
    @PutMapping("/dictionaryItem/update")
    public R<DictionaryItem> updateNbWorkOrderType(@RequestBody DictionaryItemUpdateDTO dictionaryItemUpdateDTO){
        return commonConfigurationApi.update(dictionaryItemUpdateDTO);
    }

    @ApiOperation(value = "查询报装工单类型配置")
    @RequestMapping(value = "/dictionaryItem/find", method = RequestMethod.GET)
    public R<List<DictionaryItem>>findNbWorkOrderType(@RequestParam("dictionarytype") String dictionarytype){
