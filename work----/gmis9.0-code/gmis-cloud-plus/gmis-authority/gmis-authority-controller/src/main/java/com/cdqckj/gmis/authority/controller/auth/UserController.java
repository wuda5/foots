...
import com.cdqckj.gmis.authority.controller.poi.ExcelUserVerifyHandlerImpl;
...
import com.cdqckj.gmis.common.constant.DictionaryCode;
...
public class UserController extends SuperCacheController<UserService, Long, User, UserPageDTO, UserSaveDTO, UserUpdateDTO> {
    ...
    public R<User> handlerSave(UserSaveDTO data) {
        ...
        //updater by hc 公司编码，及公司名保存
        baseService.saveUser(setCommonParams(user));
        return success(user);
    }
    ...
    @SysLog(value = "'修改用户")
    public R<User> updateUser(@RequestBody UserUpdateDTO updateDTO) {
        User user = BeanUtil.toBean(updateDTO, User.class);
        baseService.updateUser(user);
        UserOrgIdUtil.clearUserOrgIdCache();
        return success(user);
    }
...
//        params.setDictHandler();
    ...
    @GetMapping("/test")
    public String langueTest(){
       return JSON.toJSONString(payBizApi.getAllWxChangeDetial());
    }
    /**
    ...
    public void query(PageParams<UserPageDTO> params, IPage<User> page, Long defSize) {
        ...
        baseService.findPage(page, wrapper, BaseContextHandler.getTenant());
    }
}
