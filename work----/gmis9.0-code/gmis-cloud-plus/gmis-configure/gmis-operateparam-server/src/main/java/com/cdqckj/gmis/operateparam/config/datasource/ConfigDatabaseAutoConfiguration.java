package com.cdqckj.gmis.operateparam.config.datasource;
...
@Slf4j
@MapperScan(
        basePackages = {"com.cdqckj.gmis",},
        annotationClass = Repository.class,
        sqlSessionFactoryRef = ConfigDatabaseAutoConfiguration.DATABASE_PREFIX + "SqlSessionFactory")
@EnableConfigurationProperties({MybatisPlusProperties.class})
@ConditionalOnExpression("!'DATASOURCE'.equals('${gmis.database.multiTenantType}')")
public class ConfigDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    /**
     * 每个数据源配置不同即可
     */
    final static String DATABASE_PREFIX = "master";

    public ConfigDatabaseAutoConfiguration(MybatisPlusProperties properties,
