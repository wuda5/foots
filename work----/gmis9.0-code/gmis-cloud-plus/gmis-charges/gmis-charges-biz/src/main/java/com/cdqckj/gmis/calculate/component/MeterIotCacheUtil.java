...
@Slf4j
@Component
public class MeterIotCacheUtil {
    ...
    @Autowired
    private GasMeterFactoryService gasMeterFactoryBizApi;

    @Autowired
    private IotSubscribeBizApi iotSubscribeBizApi;

    @Autowired
    private GasMeterVersionService gasMeterVersionBizApi;
    ...
    public IotR matchMeterCache(MeterCacheVO meterCacheVO) {
        ...
        if(list!=null&&list.size()>0){
            for(GasMeter gasMeter:list){
                GasMeterVersion gasMeterVersion = gasMeterVersionBizApi.getById(gasMeter.getGasMeterVersionId());
                if(gasMeter.getGasMeterFactoryId()!=null&&(OrderSourceNameEnum.REMOTE_RECHARGE.getCode().equals(gasMeterVersion.getOrderSourceName().trim())||
                    ...
                    GasMeterFactory gasMeterFactory = gasMeterFactoryBizApi.getById(gasMeter.getGasMeterFactoryId());
        ...
        if(gasMeterFactoryId!=null){
            for(String meter:meterList){
                GasMeterFactory gasMeterFactory = gasMeterFactoryBizApi.getById(gasMeterFactoryId);
                if(gasMeterFactory!=null){
                    IotSubscribe iotSubscribe = iotSubscribeBizApi.
                            queryByFactoryCode(gasMeterFactory.getGasMeterFactoryCode());
