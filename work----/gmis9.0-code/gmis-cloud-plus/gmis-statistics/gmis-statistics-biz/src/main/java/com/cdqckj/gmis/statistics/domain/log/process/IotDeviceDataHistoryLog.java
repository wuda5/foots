 ...
 * @remark: 请输入类说明
 */
public class IotDeviceDataHistoryLog implements TableInsertLog<IotDeviceDataHistory> {
    ...
    public IotDeviceDataHistoryLog(Class className, SysServiceBean sysServiceBean) {
    ...
    @Override
    public Boolean logNeedProcess(FormatRowData<IotDeviceDataHistory> formatRowData, ProcessTypeEnum typeEnum) {

        IotDeviceDataHistory after = formatRowData.getAfterRowValue();
        if (after.getDataType().equals(DataType.DeviceDataNotice.getCode())){
            return true;
        }else {
            return false;
        }
    ...
    @Override
    public Boolean insertLog(FormatRowData<IotDeviceDataHistory> formatRowData) {

        IotDeviceDataHistory after = formatRowData.getAfterRowValue();
        if (after == null || after.getDeviceId() == null){
            return true;
        }
        ...
        String sql = EntityFieldUtil.searchSameRecordSql(logData, Arrays.asList("uploadTimes", "create_user_id", "alarmType"));
