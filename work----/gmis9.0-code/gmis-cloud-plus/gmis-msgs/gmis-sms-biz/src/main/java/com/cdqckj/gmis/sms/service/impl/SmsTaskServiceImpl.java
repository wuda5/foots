...
public class SmsTaskServiceImpl extends SuperServiceImpl<SmsTaskMapper, SmsTask> implements SmsTaskService {
    ...
    public void validAndInit(SmsTask smsTask, TemplateCodeType type) {
        ...
        if (type != null) {
            ...
            BizAssert.notNull(template, BASE_VALID_PARAM.build("短信参数不能为空","SMS parameter cannot be empty"));
        ...
        BizAssert.isFalse(phoneList == null || phoneList.isEmpty(),
                BASE_VALID_PARAM.build("接收人不能为空","Receiver cannot be empty"));
        ...
        if (smsTask.getSendTime() != null) {
            ...
            BizAssert.isTrue(flag,
                    BASE_VALID_PARAM.build("定时发送时间至少在当前时间的5分钟之后",
                            "The scheduled transmission time shall be at least 5 minutes later than the current time"));
        }

        if (StringUtils.isNotEmpty(smsTask.getContent()) && smsTask.getContent().length() > 450) {
            throw new BizException(BASE_VALID_PARAM.getCode(),
                    "发送内容不能超过500字","Send content cannot exceed 500 words");
        }

        String templateParams = smsTask.getTemplateParams();
        JSONObject obj = JSONObject.parseObject(templateParams, Feature.OrderedField);
        BizAssert.notNull(obj,
                BASE_VALID_PARAM.build("短信参数格式必须为严格的json字符串",
                        "SMS parameter format must be strict JSON string"));
