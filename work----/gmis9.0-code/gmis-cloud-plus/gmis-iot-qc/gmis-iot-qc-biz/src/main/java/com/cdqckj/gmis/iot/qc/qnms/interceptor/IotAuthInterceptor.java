...
public class IotAuthInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //所有带@QnmsIotAuth注解的方法均要拦截
        if (method.getAnnotation(QnmsIotAuth.class) != null) {
            ...
            if(!redisService.hasKey(RedisConstant.IOT_GMIS+StrPool.COLON+tenant+StrPool.COLON+
                ...
                redisService.set(RedisConstant.IOT_GMIS+StrPool.COLON+
                        RedisConstant.DOMAIN_ID_TENANT+StrPool.COLON+domainId,tenant);
            }
            if(redisService.hasKey(RedisConstant.IOT_GMIS+StrPool.COLON+tenant+StrPool.COLON+
                    RedisConstant.IOT_TOKEN+StrPool.COLON+domainId)){
                token = (String)redisService.get(RedisConstant.IOT_GMIS+StrPool.COLON+tenant+StrPool.COLON+
                        RedisConstant.IOT_TOKEN+StrPool.COLON+domainId);
            }
            //如果token存在放行
            ...
            //如果token不存在则先登录
                ...
                IotR r = IotSpringUtils.getBean(TokenHelper.class).login(licence);
                if(r != null && MapUtil.getInt(r, IotRConstant.CODE) == 0){
                    IotConfig iotConfig = SpringUtils.getBean(IotConfig.class);
                    token = JSONObject.parseObject(JSONObject.toJSONString(r))
                            .getJSONArray(IotRConstant.CONTENT)
                            .getJSONObject(0)
                            .getString(RequestConstant.ACCESS_TOKEN);
                    redisService.set(RedisConstant.IOT_GMIS+StrPool.COLON+tenant+StrPool.COLON+
                            RedisConstant.IOT_TOKEN+StrPool.COLON+domainId,token,iotConfig.getTokenTime());
                    return true;
                }else{
                    //返回错误信息
                    response.reset();
