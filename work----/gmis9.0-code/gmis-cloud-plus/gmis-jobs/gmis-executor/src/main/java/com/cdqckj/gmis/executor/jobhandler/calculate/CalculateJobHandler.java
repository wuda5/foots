...
@JobHandler(value = "calculateJobHandler")
@Component
@Slf4j
public class CalculateJobHandler extends IJobHandler {
    @Autowired
    ...
    public ReturnT<String> execute2(String param) throws Exception {
        ...
        if(jobParamsVO!=null){
            ...
            calculateBizApi.settlementEX(jobParamsVO.getTenantCode(),jobParamsVO.getExecuteDate());
        }else{
            List<Tenant> tenantList = tenantService.getAllTenant();
            for(Tenant tenant:tenantList){
                BaseContextHandler.setTenant(tenant.getCode());
                calculateBizApi.settlement();
            }
        }
        return SUCCESS;
    }
