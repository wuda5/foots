...
//            this.generatorApiModular(serviceName, objectMap);
        ...
        System.err.println("------------------------------------------");
        System.err.println("将下面的配置手动加入nacos中 gmis-zuul-server.yml");
        System.err.println(String.format("    %s:\n" +
                "      path: /%s/**\n" +
                "      serviceId: %s-server", config.getServiceName(), config.getServiceName(), serviceName));

        System.err.println("------------------------------------------");
        System.err.println("将下面的配置手动加入nacos中 gmis-gateway-server.yml");
...
//        this.writer(objectMap, String.format(INIT_JAVA_FTL, "GeneralController"), Paths.get(serverApplicationParentPath, "general", "controller", service + "GeneralController.java").toString());
        ...
        //数据源
        this.writer(objectMap, String.format(INIT_JAVA_FTL, "DynamicDataSourceAutoConfiguration"), Paths.get(serverConfigPath, "datasource", service + "DynamicDataSourceAutoConfiguration.java").toString());
