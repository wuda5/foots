...
spring:
  ...
  cloud:
    nacos:
      config:
        server-addr: ${r"${"}gmis.nacos.ip${r"}"}:${r"${"}gmis.nacos.port${r"}"}
        file-extension: yml
        namespace: ${r"${"}gmis.nacos.namespace${r"}"}
        shared-configs:
          - dataId: common.yml
            refresh: true
          - dataId: redis.yml
          ...
          - dataId: rabbitmq.yml
            refresh: false
        enabled: true
        username: ${r"${"}gmis.nacos.username${r"}"}
        password: ${r"${"}gmis.nacos.password${r"}"}
      discovery:
        username: ${r"${"}gmis.nacos.username${r"}"}
        password: ${r"${"}gmis.nacos.password${r"}"}
        server-addr: ${r"${"}gmis.nacos.ip}:${r"${"}gmis.nacos.port${r"}"}
        namespace: ${r"${"}gmis.nacos.namespace${r"}"}
