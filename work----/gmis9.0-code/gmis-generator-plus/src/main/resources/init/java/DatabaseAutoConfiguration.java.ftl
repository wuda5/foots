 ...
 * 此时，项目的多租户模式切换成：${r"${gmis.database.multiTenantType}"}。
 * <p>
 * NONE("非租户模式"): 不存在租户的概念
...
@Slf4j
@MapperScan(
        basePackages = { "${packageBaseParent}", <#if packageBaseParent != "com.cdqckj.gmis">"com.cdqckj.gmis"</#if>}, annotationClass = Repository.class,
        sqlSessionFactoryRef = ${service}DatabaseAutoConfiguration.DATABASE_PREFIX + "SqlSessionFactory")
@EnableConfigurationProperties({MybatisPlusProperties.class})
@ConditionalOnExpression("!'DATASOURCE'.equals('${r"${gmis.database.multiTenantType}"}')")
public class ${service}DatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    ...
    public ${service}DatabaseAutoConfiguration(MybatisPlusProperties properties,
        ...
        log.debug("检测到 gmis.database.multiTenantType!=DATASOURCE，加载了 ${service}DatabaseAutoConfiguration");
    }
