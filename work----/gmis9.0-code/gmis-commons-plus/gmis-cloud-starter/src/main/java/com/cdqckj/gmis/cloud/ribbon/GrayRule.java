package com.cdqckj.gmis.cloud.ribbon;
...
public class GrayRule extends AvailabilityFilteringRule {
    ...
    public Server choose(Object key) {
        ...
        for (Server server : serverList) {
            ...
            // version策略
            String metaVersion = metadata.get(BaseContextConstants.GRAY_VERSION);
        ...
        Map<String, List<Server>> listMap = metaServerList.stream().collect(
                Collectors.groupingBy(server -> ((NacosServer) server).getInstance().getMetadata().get(BaseContextConstants.GRAY_VERSION)));
