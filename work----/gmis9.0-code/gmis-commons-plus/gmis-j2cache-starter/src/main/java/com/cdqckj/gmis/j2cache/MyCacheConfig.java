package com.cdqckj.gmis.j2cache;
...
public class MyCacheConfig extends CachingConfigurerSupport {
    private final CacheManager j2CacheManager;

    public MyCacheConfig(CacheManager j2CacheManager) {
        this.j2CacheManager = j2CacheManager;
    }

    /**
     * 解决注解：Cacheable 没有指定key时，会将key生成为 ${value}:SimpleKey []
     * eg： @Cacheable(value = "gmis") ->  gmis:SimpleKey []
     *
     * @return
     */
    @Override
    ...
    public CacheResolver cacheResolver() {
        return (context) -> {
            ...
            for (String cacheName : cacheNames) {
                Cache cache = j2CacheManager.getCache(cacheName);
                if (cache == null) {
