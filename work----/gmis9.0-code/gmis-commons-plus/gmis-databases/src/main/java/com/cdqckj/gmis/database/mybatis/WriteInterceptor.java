package com.cdqckj.gmis.database.mybatis;
...
public class WriteInterceptor extends AbstractSqlParserHandler implements Interceptor {
    ...
    public Object intercept(Invocation invocation) {
        ...
        if (!SpringUtils.getApplicationContext().getEnvironment().getProperty("gmis.database.isNotWrite", Boolean.class, false)) {
        ...
        if (userId == 2 && (DELETE.equals(mappedStatement.getSqlCommandType()))) {
            throw new BizException(-1, "演示环境，无删除权限，请本地部署后测试","Demo environment, no delete permission, please test after local deployment");
        }
        ...
        if ("0000".equals(tenant) && isWrite) {
            throw new BizException(-1, "演示环境，无权限, 请自行创建租户后测试写入","Demo environment, no permission, please test write after creating tenant by yourself");
        }
