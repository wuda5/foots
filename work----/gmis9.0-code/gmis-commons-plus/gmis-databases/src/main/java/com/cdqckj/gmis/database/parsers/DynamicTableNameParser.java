package com.cdqckj.gmis.database.parsers;
...
public class DynamicTableNameParser implements ISqlParser {
    ...
    private String database = "gmis_base";
    ...
    public SqlInfo parser(MetaObject metaObject, String sql) {
        if (allowProcess(metaObject)) {
            ...
            // 本项目所有服务连接的默认数据库都是gmis_defaults， 不执行以下代码，将在默认库中执行sql

            // 想要 执行sql时， 不切换到 gmis_base_{TENANT} 库, 请直接返回null
            String tenantCode = BaseContextHandler.getTenant();
            if (StrUtil.isEmpty(tenantCode)) {
                return null;
            }
