package com.cdqckj.gmis.database.mybatis.auth;
...
public class DataScopeInterceptor extends AbstractSqlParserHandler implements Interceptor {
    ...
    public Object intercept(Invocation invocation) {
        ...
        //查其他--->包含CUSTOMIZE自定义 / THIS_LEVEL_CHILDREN=3本级以及子级 / THIS_LEVEL=4本级
    ...
    private DataScope findDataScopeObject(Object parameterObj) {
        if (parameterObj instanceof DataScope) {
            ...
            for (Object val : ((Map<?, ?>) parameterObj).values()) {
                ...
                if (val instanceof LbqWrapper){
                    LbqWrapper wrapper = (LbqWrapper)val;
                    DataScope dataScope = wrapper.getTheDataScope();
                    if (dataScope != null){
                        return dataScope;
                    }
                }
                if (val instanceof QueryWrap){
                    QueryWrap wrapper = (QueryWrap)val;
                    DataScope dataScope = wrapper.getTheDataScope();
                    if (dataScope != null){
                        return dataScope;
                    }
